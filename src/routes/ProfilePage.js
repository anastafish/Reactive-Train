            ...
            <Box className='absolute sm:top-[40%] sm:right-[40%] right-[15%] top-[40%]'>
                ...
                className='bg-white border-[2px] items-center gap-4
                 flex flex-col border-black p-4 rounded-sm'>
                  <TextField name='name' label={t('user_name')} value={newUserInfo.name} onChange={handleChange}/>
                  <TextField name='email' label={t('email')} value={newUserInfo.email} onChange={handleChange}/>
                    <Button
          ...
          <div className='flex flex-col items-center justify-center mt-20 gap-5 '>
            ...
            <div className='flex items-center bg-gray-400 gap-4 rounded-lg p-5'>
              ...
              <div className='flex flex-col items-center'>
                <h1 className='text-[20px]'>{t('user_name')}: {user.userInfo.name}</h1>
                <h1 className='text-[20px]'>{t('email_address')}: {user.userInfo.email}</h1>
              </div>
            </div>
              <Button onClick={() => setPopUp(true)}
               variant='contained' color='warning'>{t('edit_profile')}</Button>
               <Button onClick={() => logOut()}
               variant='contained' color='error'>{t('logout')}</Button>
          </div>
        </div>
        </div>
