...
function Reservation() {
  ...
  const cities = [t('jeddah'), t('makka'), t('madina'), t('dammmam'), t('riyadh')]
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
        <div 
        className='flex flex-col h-[100vh] w-[100vw] items-center'
        ...
        style={{zIndex:9}}
             severity="warning"
              ...
              >{error}</Alert> }
              <div              
              className='flex flex-col justify-between gap-4 p-2 items-center w-full'>
          <Nav theme={false}/>
            ...
            <div className='flex flex-col items-center justify-center p-6 sm:gap-1 gap-2'>
                  <div className='flex items-center sm:flex-row flex-col
                   w-full justify-center sm:gap-0 gap-2'>
                    <Select defaultValue={t('from')} className='sm:w-[40%] w-[70%] bg-white' name='from' onChange={handleChangeRes} value={reservation.from}>
                      <MenuItem value='From' disabled>{t("from")}</MenuItem>
                      {cities.map(city => <MenuItem key={city} value={city}>{city}</MenuItem> )}
                    </Select>
                    <Select defaultValue={'To'} className='sm:w-[40%] w-[70%] bg-white'  name='to' onChange={handleChangeRes} value={reservation.to}>
                     <MenuItem value='To' disabled>{t('to')}</MenuItem>
                    {cities
                ...
                <div className='flex w-full items-center sm:flex-row flex-col sm:gap-0 gap-2'>
                  <Select onChange={handleChange} defaultValue={'return'}
                  ...
                  <DateTimePicker
                   ...
                   />
                  {trip === 'return' && <DateTimePicker className='datetime sm:w-[40%] w-[70%]' name='return' value={reservation.return}
                  label={t('back')}
              ...
              <div className='flex flex-col sm:gap-0 gap-3 items-center '>
                ...
                <Button style={{marginBottom:'1rem', marginTop:'1rem'}} color='success' variant="contained" onClick={search}>
                  {t('search')}
                </Button>
                {/* <a
